...
import br.com.dio.reactiveflashcards.domain.exception.RetryExcepcion;
...
public class RetryHelper {
    ...
    public Retry processRetry(final String retryIdentifier, final Predicate<? super Throwable> errorFilter){
        return Retry.backoff(retryConfig.maxRetries(), retryConfig.minDurationSeconds())
                .filter(errorFilter)
                .doBeforeRetry(retrySignal -> log.warn("=== Retrying {} - {} times(s)", retryIdentifier, retrySignal.totalRetries()))
                .onRetryExhaustedThrow(((retryBackoffSpec, retrySignal) ->
                        new RetryExcepcion(GENERIC_MAX_RETRIES.getMessage(), retrySignal.failure())));
